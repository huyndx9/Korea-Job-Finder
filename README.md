...
4. [베트남·외국인 채용 전체 스캔](#4-베트남외국인-채용-전체-스캔)
5. [사이트 직접 추가](#5-사이트-직접-추가)
6. [설정](#6-설정)
7. [아키텍처](#7-아키텍처)
8. [API](#8-api)
9. [테스트](#9-테스트)
...
- **베트남·외국인 채용 전체 스캔** — 버튼 하나로 18개 한국어 키워드를 모든 소스에 훑고,
  공고 본문 기준으로 관련도를 매겨 정렬 ([자세히](#4-베트남외국인-채용-전체-스캔))
...
알바몬 · 알바천국 · 인디드 · 커리어 · 잡플래닛 · 로켓펀치 · 코워크 · K-Work · 버디즈코리아 · **리멤버**
...
**리멤버(Remember)가 대표적인 예입니다.** Cloudflare가 우리의 정직한 User-Agent
(`...KoreaJobFinder/1.0`)에 **403**을 돌려줍니다. 같은 URL에 일반 브라우저 UA를 쓰면
200이 오지만, 그것은 의도적으로 설치된 차단을 크롤러의 신분을 감춰 우회하는 것이므로
하지 않습니다. 리멤버 데이터가 필요하다면 리멤버 측에 API/제휴를 문의하는 것이 올바른
경로입니다.
...
검색창 아래 **🇻🇳 베트남·외국인 채용 전체 스캔** 버튼을 누르면, 검색어 하나가 아니라
한국 기업이 베트남인을 채용할 때 실제로 쓰는 표현 **18가지**를 모든 소스에 한 번에
훑습니다. 약 30초 걸립니다.

| 갈래 | 키워드 |
|---|---|
| 언어·국적 | 베트남어 · 베트남어 통역 · 베트남 근로자 · 베트남 |
| 외국인 채용 | 외국인 채용 · 외국인 근로자 · 외국인 가능 · 이주노동자 |
| **비자 코드** | E-9 · 고용허가제 · E-7 · F-4 · F-2 비자 · H-2 · D-10 · 비자 지원 |
| 관련 직무 | 통번역 · 다문화 |

**비자 코드가 핵심입니다.** 베트남인이 한국에서 일하는 실제 법적 경로가 비자이고
(E-9 고용허가제가 가장 흔함), 회사는 공고에 받아주는 비자를 적어둡니다. 언어 키워드만
쓰면 이런 공고를 통째로 놓칩니다. 실측 예: **"[천안] 병원 베트남 통역사 채용(E-7, D-2,
F-4, F-6 비자 가능)"** — 비자 키워드로 찾은 100점 공고입니다.

### 관련도 점수 (`vn_score`)

수집한 공고는 **검색어가 아니라 공고 본문**을 기준으로 0–100점을 매깁니다. 일반 키워드로
걸려온 공고도 실제로 외국인을 받으면 높은 점수를 받고, 반대로 무관한 공고는 걸러집니다.
40점 이상이면 카드에 🇻🇳 배지가 붙습니다.
...
| 외국인 채용·근로자·가능 | 22 |
| 고용허가제(EPS) · E-9 | 20 |
| E-7 · F-2/4/5/6 · H-2 · 비자 지원 | 15 |
| D-2/D-10 · 다문화 · 이주 | 12 |
| 통번역 · TOPIK | 10 |

실측 결과 (5개 소스, 2026-09): **903건 수집 → 중복 제거 → 275건이 베트남·외국인 관련**.

`GET /api/jobs?min_vn_score=50&sort=vn_score` 로 저장된 공고에서도 같은 기준으로
거르고 정렬할 수 있습니다.

> **"인터넷 전체를 훑는 것"은 아닙니다.** 어떤 크롤러도 그렇게 할 수 없고, 대형
> 채용 사이트 상당수는 봇 접근을 막습니다. 이 기능은 **동작하는 모든 소스**를 넓은
> 키워드로 훑는 것입니다. 더 넓히려면 새 소스를 추가하는 것이 유일한 방법입니다.
...
| `POST /api/scan` | 베트남·외국인 채용 전체 스캔 (18개 키워드) |
...
**235개 테스트. 실제 네트워크를 호출하는 테스트는 하나도 없으며, 저장소에 실제 API 키는
